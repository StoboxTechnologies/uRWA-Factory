...
| 10 | **`OfferingRegistry.createOffering` is permissionless and the treasury trusts the registry blindly** (session 11): an allow-listed wallet can drain a token's treasury through a self-made offering. Reproduced by test in a scratch copy. | **Closed, session 13 (Phase 0 · slice 1):** `createOffering` asks the token for OFFERING_OPERATOR or ISSUER_ADMIN and checks the treasury belongs to the token and to this registry; `settle` needs the raise over; `removeRule` only in Draft or Paused; `forceStatus` never leaves a final state nor enters Settled; `Treasury.refund` pays only from what the offering locked; `lockCap` honoured at creation; delivery emits `TokensDelivered`. Nine tests in `test/Phase0Money.t.sol`, each proven by reverting the fix. |
...
| 13 | **The Technical Safe is a 1-of-1 on a hot key** (session 11, third pass; read from Base): threshold 1, single owner with an EIP-7702 delegation. Granting it any STV4 role would put every token under one browser key. Also G20–G23 and the institutional register O1–O28 on `docs/architecture/orbit.html`. | Deferred by ruling (25.09.2026): a separate technical Safe later; not a launch gate. Phase 0 delivers two-step role handover and the technical/issuer role split instead, so the roles can move when the Safe is set up. Recorded as an accepted risk at mainnet (G19). The whole design set is on one page, `docs/architecture/stobox-orbit.html` (generated by `build-master.py`); Gene ruled 25.09 night: start soon, assurance on demand with no spending now, fee capability kept as a configurable policy. |
| 12 | **Gap register G1–G18** (session 11, second pass) – root role, setters outside the delay, storage writable by any facet, per-transaction investor limits, an offering state machine with back-transitions, an unused treasury reserve, mandates nobody enforces, decisions that never reach the logs, a `regime` field that sets nothing, interfaces that disagree with code. Each row cites the line. | `docs/architecture/stv4.html` §5 gap register; Phase 0 of the STV4 build order, with a failing-first test per row and the external audit before any money path ships. |
