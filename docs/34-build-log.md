...
**Later the same night.** Gene ruled: start soon, team to be named; A-05 and the Board as
recommended; assurance on demand, no spending now; the fee capability stays and becomes a
configurable fee policy (G17 re-cut). Hub v2.2, Orbit v0.4, Functions v0.3. `stobox-orbit.html`
v1.0, the whole system on one page, generated by `docs/architecture/build-master.py` from the seven
pages plus a rulings register and a roadmap by week; SVG overflow check clean, no phone-width
scroll, no duplicate ids, both themes rendered.
